...
async function main() {
  console.log('=== Resetar Visibilidade do Catálogo (CLI) ===')
  try {
  ...
  const confirm = await question(`\nIsto irá definir showInCatalog = false para TODOS os produtos da loja ${store.name}. Continuar? (s/n): `)
  ...
  while (true) {
    ...
    for (const d of snap.docs) {
      const data = d.data()
      totalChecked++
      if (data?.showInCatalog !== false) {
        batch.update(d.ref, { showInCatalog: false, updatedAt: new Date() })
        ops++
        totalUpdated++
      }
    }
    ...
    process.stdout.write(`\rVerificados: ${totalChecked} | Atualizados (showInCatalog=false): ${totalUpdated}`)
