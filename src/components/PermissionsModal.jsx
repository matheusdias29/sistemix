      ...
      <div className="bg-white rounded-lg shadow-lg w-[760px] max-w-[95vw]">
        ...
        <div className="p-4 max-h-[70vh] overflow-y-auto">
          ...
          <Section title="Vendas">
            ...
            <Switch checked={perms.sales.edit} onChange={toggle('sales','edit')} label="Pode editar vendas (histórico)" />
            ...
            <Switch checked={perms.sales.discount} onChange={toggle('sales','discount')} label="Pode aplicar desconto" />
            <Switch checked={perms.sales.fees} onChange={toggle('sales','fees')} label="Pode aplicar taxas" />
          </Section>
          ...
          <Section title="Categorias">
            ...
            <Switch checked={perms.categories.delete} onChange={toggle('categories','delete')} label="Excluir categorias" />
            <Switch checked={perms.categories.bulkPricing} onChange={toggle('categories','bulkPricing')} label="Precificação em massa" />
          </Section>
          ...
          <Section title="Fornecedores">
            ...
            <Switch checked={perms.suppliers.delete} onChange={toggle('suppliers','delete')} label="Excluir fornecedores" />
          </Section>
          ...
          <Section title="Clientes">
            ...
            <Switch checked={perms.clients.view} onChange={toggle('clients','view')} label="Visualizar clientes" />
            <Switch checked={perms.clients.delete} onChange={toggle('clients','delete')} label="Excluir clientes" />
          </Section>
          ...
          <Section title="Contas a receber">
            ...
            <Switch checked={perms.receivables.delete} onChange={toggle('receivables','delete')} label="Pode excluir contas a receber" />
          </Section>
          ...
          <Section title="Contas a pagar">
            ...
            <Switch checked={perms.payables.delete} onChange={toggle('payables','delete')} label="Pode excluir contas a pagar" />
          </Section>
          ...
          <Section title="Configurações">
            <Switch checked={perms.settings.view} onChange={toggle('settings','view')} label="Acesso geral a configurações" />
            <Switch checked={perms.settings.company} onChange={toggle('settings','company')} label="Dados da empresa" />
            <Switch checked={perms.settings.users} onChange={toggle('settings','users')} label="Gerenciar usuários" />
            <Switch checked={perms.settings.payments} onChange={toggle('settings','payments')} label="Formas de pagamento" />
            <Switch checked={perms.settings.taxes} onChange={toggle('settings','taxes')} label="Taxas adicionais" />
            <Switch checked={perms.settings.commissions} onChange={toggle('settings','commissions')} label="Configurar comissões" />
            <Switch checked={perms.settings.os} onChange={toggle('settings','os')} label="Configurar O.S" />
          </Section>
