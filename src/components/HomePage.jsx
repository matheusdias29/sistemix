...
export default function HomePage({ storeId, onNavigate, onOpenSalesDay, user }){
    ...
    relevant.forEach(g => {
      ...
      const currentVal = monthOrders.reduce((acc, o) => {
        ...
        if (g.includeServiceOrder && isOS) {
           // Para metas de O.S. por vendedor, contabilizar pela pessoa no campo Atendente
           if (o.attendantId && o.attendantId === g.sellerId) match = true
           else {
             const name = o.attendant || o.attendantName
             if (name && name === g.sellerName) match = true
  ...
  return (
    <div className="space-y-6">
      {/* Resumo do Dia */}
      {(isOwner || perms.sales?.viewAll || perms.sales?.view || perms.serviceOrders?.view) && (
      <section className="rounded-xl bg-white dark:bg-gray-800 p-5 md:p-8 shadow-sm border border-gray-100 dark:border-gray-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Card Total de Venda */}
            {(isOwner || perms.sales?.viewAll || perms.sales?.view) && (
            <button
