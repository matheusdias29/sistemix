...
export default function ShareSaleModal({ open, onClose, sale, store }) {
  if (!open || !sale) return null

  const saleNumber = sale.number ? String(sale.number).replace(/\D/g, '') : String(sale.id).slice(-4)
  const displayId = `PV:${String(saleNumber).padStart(4, '0')}`
  const [whatsapp, setWhatsapp] = useState('')

  const shareUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}/comprovantes/venda/${sale.id}` 
    : ''
  ...
  const handleWhatsapp = () => {
    ...
    const text = encodeURIComponent(`Olá, segue o link do comprovante da sua compra na ${storeName}: ${shareUrl}`)
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Venda ${displayId}`,
          text: `Comprovante da venda ${displayId}`,
          url: shareUrl
        })
      } catch (err) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="p-6 flex flex-col items-center">
          <h2 className="text-xl font-semibold text-gray-800 mb-6">{displayId}</h2>
